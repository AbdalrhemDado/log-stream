...
- **Implementation and validation stages:** Stages 4 and 9 after approval
...
**PROPOSED — not approved:** use typed-array `UNNEST` as the initial bulk method. Internally compare chunk sizes such as 500, 1,000, and 5,000; these are not public batch limits. Multiple chunks for one accepted request execute in one transaction.

Use one shared `pg` pool with proposed maximum 4, then measure 2/4/8. Migrations acquire a client before traffic; retention shares the pool and skips work when its advisory lock is unavailable. Evaluate `COPY` only if durable `UNNEST` evidence misses the target or shows excessive CPU/memory.
...
1. Approve `UNNEST` as the initial method and `COPY` as a gated experiment?
2. Approve a shared pool baseline of four connections?
