...
import { LOG_LEVELS, type LogLevel, type ValidatedLogEntry } from "./log-entry.js";
import {
...
function parseTimestamp(value: string): TimestampParseResult {
  ...
  if (!result.ok && result.kind === "grammar") {
...
export function validateLogEntry(input: unknown, referenceTimeMs: number): ValidationResult {
  ...
  if (isParsedTimestampAfterEpochMilliseconds(timestamp.value, referenceTimeMs + FIVE_MINUTES_MS)) {
